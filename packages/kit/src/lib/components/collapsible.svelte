<script lang="ts">
	...
	let {
		...
		contentProps?: ComponentProps<typeof Collapsible.Content>;
		triggerProps?: ComponentProps<typeof Collapsible.Trigger>;
	} = $props();
...
<Collapsible.Root bind:open class={cn("mt-3 rounded-lg border", className)}>
	...
	<Collapsible.Content {...contentProps} class={cn("px-4 pb-4 [&>p]:mt-4", contentProps?.class)}>
		{@render children?.()}
	</Collapsible.Content>
</Collapsible.Root>
